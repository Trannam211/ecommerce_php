...
if(isset($_POST['form1'])) {
	...
	if($_POST['p_code'] === '') {
		...
		$statement->execute(array($_POST['p_code'], $_REQUEST['id']));
	...
	$existing_color_photo_count = array();
	if(count($photo_color_selected) > 0) {
		$statement_color_photo = $pdo->prepare("SELECT color_id, COUNT(*) AS total FROM tbl_product_photo WHERE p_id=? AND color_id IS NOT NULL AND color_id > 0 GROUP BY color_id");
		$statement_color_photo->execute(array($_REQUEST['id']));
		$result_color_photo = $statement_color_photo->fetchAll(PDO::FETCH_ASSOC);
		foreach($result_color_photo as $row_color_photo) {
			$cid = isset($row_color_photo['color_id']) ? (int)$row_color_photo['color_id'] : 0;
			if($cid > 0) {
				$existing_color_photo_count[$cid] = isset($row_color_photo['total']) ? (int)$row_color_photo['total'] : 0;
			}
		}
	}

	$existing_color_photo_ids = array_map('intval', array_keys($existing_color_photo_count));
	$effective_photo_color_ids = array_values(array_unique(array_merge($photo_color_selected, $uploaded_color_photo_ids, $existing_color_photo_ids)));
	...
	for($i=0;$i<count($effective_photo_color_ids);$i++) {
		...
		$existingCount = isset($existing_color_photo_count[$cid]) ? (int)$existing_color_photo_count[$cid] : 0;
		if(($uploadedCount + $existingCount) <= 0) {
			$valid = 0;
	...
	if($valid == 1) {
		...
		if( isset($_FILES['photo']["name"]) && isset($_FILES['photo']["tmp_name"]) ) {
			...
			product_save_photo_items($pdo, (int)$_REQUEST['id'], $photo_names, $photo_tmp, $photo_errors, $photo_sizes, null);
		}
		...
		for($i=0;$i<count($variant_rows);$i++) {
			...
			$statement->execute(array($_REQUEST['id'], $v_size_id, $v_color_id, $v_qty));
		}
		...
		if(isset($_FILES['color_photo']) && isset($_FILES['color_photo']['name']) && is_array($_FILES['color_photo']['name'])) {
			foreach($_FILES['color_photo']['name'] as $raw_color_id => $color_files) {
				...
				product_save_photo_items($pdo, (int)$_REQUEST['id'], array_values($color_files), array_values($tmp_color_files), array_values($err_color_files), array_values($size_color_files), $color_id);
			}
		}
...
.product-edit-form .control-label {
	font-weight: 600;
	color: #2f3b52;
}

.product-edit-form .form-section-card {
...
.product-edit-form .section-title {
	margin: 0 0 14px;
	font-size: 17px;
	font-weight: 700;
	color: #243447;
...
.product-edit-form .select2-container--default .select2-selection--single,
.product-edit-form .select2-container--default .select2-selection--multiple,
...
.product-edit-form .product-multiselect {
	min-height: 220px !important;
	padding: 6px 10px;
	overflow-y: auto;
}

.product-edit-form .product-multiselect option {
	padding: 4px 6px;
}

.product-edit-form .hint-text {
...
.product-edit-form .status-select {
	max-width: 190px;
}

.product-edit-form .featured-preview,
.product-edit-form .other-photo-preview {
...
.product-edit-form .variant-check-grid {
	display: grid;
...
.product-edit-form .variant-check-item {
	display: flex;
	align-items: center;
	gap: 8px;
	font-weight: 500;
...
.product-edit-form .variant-table-wrap {
	border: 1px solid #dde3ec;
	background: #fff;
	padding: 10px;
	overflow-x: auto;
...
.product-edit-form .status-inline-grid {
	display: grid;
	grid-template-columns: 1fr 1fr;
	gap: 12px;
}

.product-edit-form .status-inline-item label {
	display: block;
	margin-bottom: 6px;
	font-size: 12px;
	font-weight: 600;
...
.product-edit-form .variant-table {
	width: 100%;
	min-width: 560px;
	margin: 0;
}

.product-edit-form .variant-table th,
.product-edit-form .variant-table td {
	vertical-align: middle;
	white-space: nowrap;
}

.product-edit-form .variant-empty {
	margin: 0;
	padding: 10px;
...
.product-edit-form .color-photo-block {
	border: 1px solid #dde3ec;
	background: #fcfdff;
	padding: 10px;
	margin-bottom: 10px;
...
.product-edit-form .color-photo-title {
	font-weight: 700;
	margin-bottom: 6px;
}

.product-edit-form .product-form-columns {
	display: block;
}

.product-edit-form .product-richtext-grid {
	display: block;
}
...
@media (min-width: 1200px) {
	...
	.product-edit-form .product-form-col-main {
		flex: 1 1 0;
		min-width: 0;
	}

	.product-edit-form .product-form-col-side {
		flex: 1 1 0;
		min-width: 0;
	}

	.product-edit-form .product-form-col .form-group {
		margin-left: 0;
		margin-right: 0;
	}

	.product-edit-form .product-form-col .col-sm-2,
	.product-edit-form .product-form-col .col-sm-3,
	.product-edit-form .product-form-col .col-sm-4,
	.product-edit-form .product-form-col .col-sm-6,
	.product-edit-form .product-form-col .col-sm-8 {
		float: none;
		width: 100%;
		padding-left: 0;
		padding-right: 0;
	...
	.product-edit-form .product-form-col .control-label {
		text-align: left;
		padding-top: 0;
		margin-bottom: 6px;
	}

	.product-edit-form .product-form-col .status-select {
		max-width: 100%;
	}

	.product-edit-form .product-form-actions {
		border-top: 0;
		padding-top: 0;
		margin-top: 8px;
	}
	...
	.product-edit-form .product-richtext-grid .form-group {
		margin-bottom: 0;
	}
}

@media (min-width: 768px) {
	.product-edit-form .box-body {
		max-width: 1280px;
		margin: 0 auto;
	}

	.product-edit-form .col-sm-4 {
		width: 78%;
	}

	.product-edit-form .col-sm-8 {
		width: 78%;
	}
}
</style>
...
foreach ($result as $row) {
	$p_name = $row['p_name'];
	$p_code = isset($row['p_code']) ? $row['p_code'] : '';
	$p_unit = isset($row['p_unit']) ? $row['p_unit'] : 'sp';
	$p_old_price = $row['p_old_price'];
	$p_cost_price = isset($row['p_cost_price']) ? $row['p_cost_price'] : '0';
	$p_profit_percent = isset($row['p_profit_percent']) ? $row['p_profit_percent'] : '30';
	$p_low_stock_threshold = isset($row['p_low_stock_threshold']) ? $row['p_low_stock_threshold'] : '5';
	$p_current_price = $row['p_current_price'];
	$p_qty = $row['p_qty'];
	$p_featured_photo = $row['p_featured_photo'];
	$p_description = admin_richtext_to_plaintext($row['p_description']);
	$p_short_description = admin_richtext_to_plaintext($row['p_short_description']);
	$p_feature = admin_richtext_to_plaintext($row['p_feature']);
	$p_condition = admin_richtext_to_plaintext($row['p_condition']);
	$p_return_policy = admin_richtext_to_plaintext($row['p_return_policy']);
...
if(isset($_POST['form1'])) {
	$p_name = isset($_POST['p_name']) ? $_POST['p_name'] : $p_name;
	$p_code = isset($_POST['p_code']) ? $_POST['p_code'] : $p_code;
	$p_unit = isset($_POST['p_unit']) ? $_POST['p_unit'] : $p_unit;
	$p_old_price = isset($_POST['p_old_price']) ? $_POST['p_old_price'] : $p_old_price;
	$p_cost_price = isset($_POST['p_cost_price']) ? $_POST['p_cost_price'] : $p_cost_price;
	$p_profit_percent = isset($_POST['p_profit_percent']) ? $_POST['p_profit_percent'] : $p_profit_percent;
	$p_current_price = isset($_POST['p_current_price']) ? $_POST['p_current_price'] : $p_current_price;
	$p_low_stock_threshold = isset($_POST['p_low_stock_threshold']) ? $_POST['p_low_stock_threshold'] : $p_low_stock_threshold;
	if($p_unit === '') {
		$p_unit = 'sp';
	}
}
...
if(isset($_POST['form1'])) {
	$variant_payload = product_collect_variant_rows($_POST);
	$variant_form_rows = $variant_payload['rows'];
}

$common_photo_rows = array();
$color_photo_rows = array();
$statement = $pdo->prepare("SHOW COLUMNS FROM tbl_product_photo LIKE 'color_id'");
$statement->execute();
if($statement->rowCount() > 0) {
	...
	foreach($result as $row) {
		...
		if($cid > 0) {
			if(!isset($color_photo_rows[$cid])) {
				$color_photo_rows[$cid] = array();
			}
			$color_photo_rows[$cid][] = $row;
		} else {
			$common_photo_rows[] = $row;
		}
	}
}

$color_photo_seed = array();
foreach($color_photo_rows as $cid => $photo_rows) {
	$color_photo_seed[$cid] = array();
	foreach($photo_rows as $photo_row) {
		$color_photo_seed[$cid][] = array(
			'pp_id' => (int)$photo_row['pp_id'],
...
for($i=0;$i<count($variant_form_rows);$i++) {
	$cid = isset($variant_form_rows[$i]['color_id']) ? (int)$variant_form_rows[$i]['color_id'] : 0;
	if($cid > 0) {
		$photo_color_selected_map[$cid] = $cid;
	}
}

// Keep colors inferred from color-tagged photos.
foreach(array_keys($color_photo_rows) as $cid_key) {
	$cid = (int)$cid_key;
	if($cid > 0) {
		$photo_color_selected_map[$cid] = $cid;
	}
...
$photo_color_selected = array_values($photo_color_selected_map);
sort($photo_color_selected);
if(isset($_POST['form1'])) {
	$photo_color_selected = product_to_int_array(isset($_POST['photo_color_selected']) ? $_POST['photo_color_selected'] : array());
}
?>
...
<section class="content">
	...
	<div class="row">
		<div class="col-md-12">
			...
			<form class="form-horizontal product-edit-form" action="" method="post" enctype="multipart/form-data">
							...
							<div class="product-form-col product-form-col-main">
								<div class="form-section-card">
									...
									<div class="form-group">
										...
										<div class="col-sm-4">
											<input type="text" name="p_name" class="form-control" value="<?php echo htmlspecialchars((string)$p_name, ENT_QUOTES, 'UTF-8'); ?>">
										</div>
									</div>
									<div class="form-group">
										<label for="" class="col-sm-3 control-label">Mã sản phẩm <span>*</span></label>
										<div class="col-sm-4">
											<input type="text" name="p_code" class="form-control" value="<?php echo htmlspecialchars((string)$p_code, ENT_QUOTES, 'UTF-8'); ?>">
										</div>
									</div>
									<div class="form-group">
										<label for="" class="col-sm-3 control-label">Đơn vị tính</label>
										<div class="col-sm-4">
											<input type="text" name="p_unit" class="form-control" value="<?php echo htmlspecialchars((string)$p_unit, ENT_QUOTES, 'UTF-8'); ?>">
										</div>
									</div>
								</div>
								...
								<div class="form-section-card">
									...
									<div class="form-group">
										...
										<div class="col-sm-4" style="padding-top:4px;">
											<img src="../assets/uploads/<?php echo $p_featured_photo; ?>" alt="" class="featured-preview">
								...
								<div class="form-section-card">
									...
									<div class="form-group">
										...
										<div class="col-sm-4">
											<input type="text" name="p_cost_price" class="form-control currency-input" inputmode="numeric" value="<?php echo ($p_cost_price !== '' ? number_format((float)$p_cost_price,0,'.',',') : ''); ?>" oninput="this.value=this.value.replace(/[^0-9]/g,'').replace(/\B(?=(\d{3})+(?!\d))/g,',');">
										</div>
									</div>
									<div class="form-group">
										<label for="" class="col-sm-3 control-label">% lợi nhuận <span>*</span></label>
										<div class="col-sm-4">
											<input type="text" name="p_profit_percent" class="form-control" value="<?php echo htmlspecialchars((string)$p_profit_percent, ENT_QUOTES, 'UTF-8'); ?>" inputmode="decimal">
										</div>
									</div>
									<div class="form-group">
										<label for="" class="col-sm-3 control-label">Giá bán <span>*</span></label>
										<div class="col-sm-4">
											<input type="text" name="p_current_price" class="form-control currency-input" inputmode="numeric" value="<?php echo ($p_current_price !== '' ? number_format((float)$p_current_price,0,'.',',') : ''); ?>" readonly>
									...
									<div class="form-group">
										...
										<div class="col-sm-4">
											<input type="text" name="p_old_price" class="form-control currency-input" inputmode="numeric" value="<?php echo ($p_old_price !== '' ? number_format((float)$p_old_price,0,'.',',') : ''); ?>" oninput="this.value=this.value.replace(/[^0-9]/g,'').replace(/\B(?=(\d{3})+(?!\d))/g,',');">
										</div>
									</div>
									<div class="form-group">
										<label for="" class="col-sm-3 control-label">Ngưỡng tồn kho thấp</label>
										<div class="col-sm-4">
											<input type="number" min="0" name="p_low_stock_threshold" class="form-control" value="<?php echo (int)$p_low_stock_threshold; ?>">
										</div>
									</div>
									<div class="form-group">
										<label for="" class="col-sm-3 control-label"></label>
										<div class="col-sm-8" style="padding-top:8px;">
											<input type="hidden" name="p_qty" value="<?php echo (int)$p_qty; ?>">
										</div>
									</div>
								</div>
								...
								<div class="form-section-card">
									...
									<div class="product-richtext-grid">
										<div class="form-group">
											...
											<div class="col-sm-8">
												<textarea name="p_description" class="form-control" cols="30" rows="10" id="editor1"><?php echo htmlspecialchars($p_description, ENT_QUOTES, 'UTF-8'); ?></textarea>
											</div>
										</div>
										<div class="form-group">
											<label for="" class="col-sm-3 control-label">Mô tả ngắn</label>
											<div class="col-sm-8">
												<textarea name="p_short_description" class="form-control" cols="30" rows="10" id="editor2"><?php echo htmlspecialchars($p_short_description, ENT_QUOTES, 'UTF-8'); ?></textarea>
											</div>
										</div>
										<div class="form-group">
											<label for="" class="col-sm-3 control-label">Đặc điểm nổi bật</label>
											<div class="col-sm-8">
												<textarea name="p_feature" class="form-control" cols="30" rows="10" id="editor3"><?php echo htmlspecialchars($p_feature, ENT_QUOTES, 'UTF-8'); ?></textarea>
											</div>
										</div>
										<div class="form-group">
											<label for="" class="col-sm-3 control-label">Hướng dẫn chọn size</label>
											<div class="col-sm-8">
												<textarea name="p_condition" class="form-control" cols="30" rows="10" id="editor4"><?php echo htmlspecialchars($p_condition, ENT_QUOTES, 'UTF-8'); ?></textarea>
											</div>
										</div>
										<div class="form-group">
											<label for="" class="col-sm-3 control-label">Chính sách đổi trả</label>
											<div class="col-sm-8">
												<textarea name="p_return_policy" class="form-control" cols="30" rows="10" id="editor5"><?php echo htmlspecialchars($p_return_policy, ENT_QUOTES, 'UTF-8'); ?></textarea>
											</div>
										</div>
									</div>
								</div>
...
(function() {
	...
	var colorPhotoSeed = <?php echo json_encode($color_photo_seed, JSON_UNESCAPED_UNICODE); ?>;
	...
	function renderVariantColorOptionList(selectedId) {
		...
		for(var j=0;j<colorOptions.length;j++) {
			// Keep saved selection visible on edit even before allowed-color rows are fully hydrated.
			if(!allowMap[colorOptions[j].id] && selectedId !== colorOptions[j].id) {
				continue;
			}
		...
		if(selectedId > 0 && !hasSelectedOption) {
			html += '<option value="' + selectedId + '" selected>Màu đã lưu (' + selectedId + ')</option>';
		}
		return html;
	}
	...
	function addVariantRow(seed) {
		...
		row.innerHTML = '' +
			...
			'<td><select name="variant_color_id[]" data-seed-color-id="' + (colorId > 0 ? colorId : 0) + '" class="form-control variant-color-select">' + renderVariantColorOptionList(colorId) + '</select></td>' +
	...
	function updatePhotoRowInput(row) {
		...
		// Preserve selected files when UI refreshes without color change.
		...
		var oldPhotos = colorPhotoSeed[String(colorId)] || colorPhotoSeed[colorId] || [];
		if(oldPhotos.length > 0) {
			html += '<div style="margin-top:8px;">';
			for(var j=0;j<oldPhotos.length;j++) {
				html += '<span style="display:inline-block;margin-right:8px;">' +
					'<img src="../assets/uploads/product_photos/' + encodeURIComponent(oldPhotos[j].photo) + '" alt="" class="other-photo-preview">' +
					'<a onclick="return confirmDelete();" href="product-other-photo-delete.php?id=' + oldPhotos[j].pp_id + '&id1=<?php echo (int)$_REQUEST['id']; ?>" class="btn btn-danger btn-xs">X</a>' +
				'</span>';
			}
			html += '</div>';
		}
	...
	function refreshPhotoColorSelects() {
		...
		for(var i=0;i<selects.length;i++) {
			...
			selects[i].setAttribute('data-prev-color-id', oldValue > 0 ? String(oldValue) : '0');
			updatePhotoRowInput(selects[i].closest('tr'));
		}
	}

	function migrateVariantColorSelection(oldColorId, newColorId) {
		if(oldColorId <= 0 || newColorId <= 0 || oldColorId === newColorId) {
			return;
		}

		function findDuplicatePair(sizeId, colorId, exceptRow) {
			var rows = document.querySelectorAll('#variantTableBody tr');
			for(var r=0;r<rows.length;r++) {
				if(rows[r] === exceptRow) {
					continue;
				}
				var sizeSelect = rows[r].querySelector('select[name="variant_size_id[]"]');
				var colorSelect = rows[r].querySelector('select[name="variant_color_id[]"]');
				var rowSize = parseInt(sizeSelect && sizeSelect.value ? sizeSelect.value : '0', 10);
				var rowColor = parseInt(colorSelect && colorSelect.value ? colorSelect.value : '0', 10);
				if(rowSize > 0 && rowColor > 0 && rowSize === sizeId && rowColor === colorId) {
					return true;
				}
			}
			return false;
		...
		for(var i=0;i<selects.length;i++) {
			var current = parseInt(selects[i].value || '0', 10);
			if(current !== oldColorId) {
				continue;
			}

			var row = selects[i].closest('tr');
			var sizeSelect = row ? row.querySelector('select[name="variant_size_id[]"]') : null;
			var sizeId = parseInt(sizeSelect && sizeSelect.value ? sizeSelect.value : '0', 10);

			var hasNewColorOption = false;
			for(var j=0;j<selects[i].options.length;j++) {
				if(parseInt(selects[i].options[j].value || '0', 10) === newColorId) {
					hasNewColorOption = true;
					break;
				}
			}

			if(!hasNewColorOption) {
				selects[i].value = '';
				continue;
			}

			if(sizeId > 0 && findDuplicatePair(sizeId, newColorId, row)) {
				selects[i].value = '';
				hasConflict = true;
				continue;
			}

			selects[i].value = String(newColorId);
			selects[i].setAttribute('data-seed-color-id', String(newColorId));
		}

		if(hasConflict) {
			alert('Có dòng biến thể bị trùng size-màu sau khi đổi màu. Hệ thống đã để trống màu ở các dòng trùng, vui lòng chọn lại màu khác.');
		}
	}
	...
	function renderColorPhotoBlocks() {
		...
		var html = '';
		html += '<table class="table table-bordered variant-table" id="colorPhotoTable">';
		html += '<thead><tr><th>Màu</th><th>Ảnh theo màu</th><th>Thao tác</th></tr></thead><tbody id="colorPhotoTableBody"></tbody></table>';
		html += '<div style="text-align:right;margin-top:8px;"><button type="button" class="btn btn-primary btn-sm" id="colorPhotoAddRowBtn">Thêm</button></div>';
	...
	function refreshVariantColorSelects() {
		...
		for(var i=0;i<selects.length;i++) {
			var oldValue = parseInt(selects[i].value || '0', 10);
			if(oldValue <= 0) {
				oldValue = parseInt(selects[i].getAttribute('data-seed-color-id') || '0', 10);
			}
			...
			if(!hasValue) {
				selects[i].value = '';
				selects[i].setAttribute('data-seed-color-id', '0');
			} else {
				selects[i].value = String(oldValue);
				selects[i].setAttribute('data-seed-color-id', String(oldValue));
			}
		}
	}
	...
	document.addEventListener('input', function(e) {
		...
		if(e.target && e.target.classList.contains('photo-color-select')) {
			var oldColorId = parseInt(e.target.getAttribute('data-prev-color-id') || '0', 10);
			var newColorId = parseInt(e.target.value || '0', 10);
			updatePhotoRowInput(e.target.closest('tr'));
			refreshPhotoColorSelects();
			refreshVariantColorSelects();
			migrateVariantColorSelection(oldColorId, newColorId);
			e.target.setAttribute('data-prev-color-id', newColorId > 0 ? String(newColorId) : '0');
		}
